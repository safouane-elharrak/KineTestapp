    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('appointment_time') ? 'is-invalid' : '' }}" type="text" name="appointment_time" id="appointment_time" value="{{ old('appointment_time') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('appointment_type') ? 'is-invalid' : '' }}" name="appointment_type" id="appointment_type">
                    ...
                    @foreach(App\Models\Appointment::APPOINTMENT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('appointment_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('appointment_status') ? 'is-invalid' : '' }}" name="appointment_status" id="appointment_status">
                    ...
                    @foreach(App\Models\Appointment::APPOINTMENT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('appointment_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('appointment_note') ? 'is-invalid' : '' }}" type="text" name="appointment_note" id="appointment_note" value="{{ old('appointment_note', '') }}">
