    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payement_method') ? 'is-invalid' : '' }}" name="payement_method" id="payement_method" required>
                    ...
                    @foreach(App\Models\Payment::PAYEMENT_METHOD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payement_method', 'cash') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_ref') ? 'is-invalid' : '' }}" type="text" name="payment_ref" id="payment_ref" value="{{ old('payment_ref', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payement_amount') ? 'is-invalid' : '' }}" type="number" name="payement_amount" id="payement_amount" value="{{ old('payement_amount', '') }}" step="0.01" required>
