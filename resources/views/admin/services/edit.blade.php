    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.services.update", [$service->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('service_reference') ? 'is-invalid' : '' }}" type="text" name="service_reference" id="service_reference" value="{{ old('service_reference', $service->service_reference) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('service_active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="service_active" id="service_active" value="1" {{ $service->service_active || old('service_active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('service_price') ? 'is-invalid' : '' }}" type="number" name="service_price" id="service_price" value="{{ old('service_price', $service->service_price) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('service_min_price') ? 'is-invalid' : '' }}" type="number" name="service_min_price" id="service_min_price" value="{{ old('service_min_price', $service->service_min_price) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('service_max_price') ? 'is-invalid' : '' }}" type="text" name="service_max_price" id="service_max_price" value="{{ old('service_max_price', $service->service_max_price) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id">
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $service->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
