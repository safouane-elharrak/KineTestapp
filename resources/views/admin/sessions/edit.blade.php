    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sessions.update", [$session->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('session_date') ? 'is-invalid' : '' }}" type="text" name="session_date" id="session_date" value="{{ old('session_date', $session->session_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('patient') ? 'is-invalid' : '' }}" name="patient_id" id="patient_id">
                    @foreach($patients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('patient_id') ? old('patient_id') : $session->patient->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('session_type') ? 'is-invalid' : '' }}" name="session_type" id="session_type">
                    ...
                    @foreach(App\Models\Session::SESSION_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('session_type', $session->session_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('session_location') ? 'is-invalid' : '' }}" type="text" name="session_location" id="session_location" value="{{ old('session_location', $session->session_location) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employe') ? 'is-invalid' : '' }}" name="employe_id" id="employe_id">
                    @foreach($employes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('employe_id') ? old('employe_id') : $session->employe->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('session_total') ? 'is-invalid' : '' }}" type="number" name="session_total" id="session_total" value="{{ old('session_total', $session->session_total) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('session_status') ? 'is-invalid' : '' }}" name="session_status" id="session_status">
                    ...
                    @foreach(App\Models\Session::SESSION_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('session_status', $session->session_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
